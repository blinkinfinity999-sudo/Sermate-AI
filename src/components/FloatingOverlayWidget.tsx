...
export const FloatingOverlayWidget: React.FC<FloatingOverlayWidgetProps> = ({
  ...
  return (
    <div className={`fixed z-50 transition-all duration-300 ${getPositionClass()}`}>
      {!isOpen ? (
        /* Floating Minimized Bubble Trigger */
        <button
          id="btn-floating-bubble"
          onClick={() => {
            if (theme.soundEnabled) sound.playPing();
            onToggleOpen();
          }}
          className="group relative flex items-center gap-2.5 px-4 py-2.5 rounded-full shadow-2xl transition-all hover:scale-105 active:scale-95 cursor-pointer border backdrop-blur-xl"
          style={{
        ...
        >
          {/* Top Titlebar with minimize & close */}
